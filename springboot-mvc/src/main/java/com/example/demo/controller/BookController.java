...
@CrossOrigin(origins = "http://localhost:5173") // 允許你的前端網址 允許跨域請求
@RestController
@RequestMapping("/book")
public class BookController {
	...
	public ResponseEntity<ApiResponse<Book>> updateBookNameAndPrice(@PathVariable Integer id, @RequestBody Book book) {
		try {
			...
			return ResponseEntity.ok(ApiResponse.success("修改書名價格成功", book));
		} catch (BookException e) {
			return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
		}
	}
	...
	public ResponseEntity<ApiResponse<Book>> updateBookName(@PathVariable Integer id, @RequestBody Book book) {
		try {
			...
			return ResponseEntity.ok(ApiResponse.success("修改書籍書名成功", book));
		} catch (BookException e) {
			return ResponseEntity.badRequest().body(ApiResponse.error(e.getMessage()));
		}
	}
