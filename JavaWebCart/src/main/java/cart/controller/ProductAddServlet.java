...
public class ProductAddServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 重導到　result.jsp
		String meassage = String.format("商品新增成功<p/> 商品名稱: %s<p/> 商品價格:%s<p/> 商品庫存:%s<p/> 商品照片:<img src='data:image/png;base64,%s'> <p/>",
				productname,price,qty,productImageBase64);
		req.setAttribute("resultTitle", "商品新增");
		req.setAttribute("resultMessage", meassage);
